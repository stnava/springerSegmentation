...
The problem of segmenting brain image voxel into distinct tissue classes is inherently a pixel- or voxel-wise classification problem. The input to the model is a 2D slice of a 3D brain image, and the model outputs a 2D image of equal shape but with each pixel assigned to one of the possible tissue classes.

In this work, we use an ensemble of three 2D convolutional-deconvolutional architectures, each trained on full image slices belonging exclusively to either the axial, saggittal, or coronal plane. Our work is one of the few to actually train on entire slices, wheras previous work in this area has mostly involved training on image patches. The slice-based approach is undoubedtly the preferred method, as the patch-based approach is computationally infeasible for segmenting entire images (see later section).
