...
1. *Data curation:* selection of data that is representative of raw data, $\{ x_i \}$, but also annotated with useful evaluation targets, $\{ y_i \}$; 

2. *Problem definition:* specific question that can be answered, given the data, and the associated (2.1) biological and other metrics and (2.2) objective (loss) functions defining the learning problem i.e. $f( x_i ) = y_i$; 

3. *Split data:* divide data into train, test and validation sets perhaps with some effort to balance the categories; 

4. *Redesign/refine:* improve the proposed system(s) with respect to cross-validation performance; Explore (4.1) deep learning architectures; (4.2) alternative optimizers; (4.3) additional pre-processing; (4.4) generalizability via supplemental datasets, if available;
...
dd=read.csv(".//Cognitive_data/Subject_Information.csv")
haveImage=rep(NA, nrow( dd ) )
for ( i in 1:nrow( dd ) ) {
  ifn = paste("/Users/bavants/Downloads/DLBSsegs/", dd$INDI_ID[i], "*nii.gz", sep='' )
  findimage = Sys.glob( ifn )
  haveImage[ i ] = length( findimage ) > 0
}
demog = dd[haveImage,]
...
for ( nm in colnames( demog )[c(3,8)] ) {
  print( paste( nm ) )
  print( knitr::kable( table( demog[,nm] )  ) )
}
```

### Imaging data

We demonstrate our application on the T1-weighted neuroimages collected in the Dallas Lifespan Brain Study (DLBS) available [here](http://fcon_1000.projects.nitrc.org/indi/retro/dlbs.html) and described in [@LuXuRodrigueEtAl2011]. The DLBS MRI dataset includes 275 subjects with 1mm$^3$ T1-weighted MPRAGE SENSE MRI collected on a 3 T Philips Medical
System machine as described [here](ftp://www.nitrc.org/fcon_1000/htdocs/indi/retro/dlbs_content/dlbs_scan_params_anat.pdf). The scanning session used a whole body coil to transmit the RF excitation and an 8-channel receive head coil with parallel imaging. The cohort subjects have average age 55.25 $\pm$ 20 years (min 20.6, max 89) and includes 107 females and 103 males with mean educational attainment of 16.3 $\pm$ 2.3 years.
...
selection of data that is representative of raw data, $\{ x_i \}$, but also annotated with useful evaluation targets, $\{ y_i \}$; 
...
such datasets do not require labels, if one is able to ...
