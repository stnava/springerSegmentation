...
FIXME --- need to specify details of the GPU in this section
...
For evaluation, we calculated the <i>Dice Coefficient</i> overlap for each tissue in the test image. We report these results here:

<!-- discuss how dice is calculated. report dice coef results. -->
...
Most importantly, we note that it takes the ensemble of models a total of between 15-30 seconds for a single subject. Since the model is already trained, it will <i>always</i> take between 15-30 seconds for any new images. This is a main benefit of prediction, since if we can prove our model generalizes across datasets, then users will have an accurate and fast segmentation model that will apply to any arbitrary dataset.
...
With a set of test predictions and evaluation metrics, we finally moved on to the interpetation phase. In this phase, we asked oruselves what our evaluation results mean for the general viability of this model.

* independent validation - results are good and the algorithm is fast.  EXPAND ON THIS.

Additionally, we want to discuss our thoughts for why this model performs so well, and why it is biologically plausible. We approached this question from a theoretical and practical standpoint.
