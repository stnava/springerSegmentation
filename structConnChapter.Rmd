...
Before proceeding to a more complex brain mapping study, we introduce a step-by-step example via a structural population dataset available in ANTsR.  By normalizing these brain slices to a common template, we can generate intuition about SCCAN covariance patterns via fast and easy to reproduce examples.  We first define the six subjects and use the first as template.
...
maskX = getMask( ref )  %>% resampleImage(  c( 2, 2 ) ) %>% iMath("MD",2)
maskY = resampleImage( maskX, c( 4, 4 ) )
...
for ( i in 1:length( fns ) )
  ...
  jac = createJacobianDeterminantImage( ref, reg$fwdtransforms[1], 1 ) %>%
    resampleImageToTarget( maskX )
  X[ i, ] = jac[ maskX == 1 ]
  tgr = antsApplyTransforms( ref, tar, reg$fwdtransforms[2] ) %>% 
    resampleImageToTarget( maskY )
  reftardif = abs( refsub - tgr )
...
between these measurements in a lower dimensional space, via SCCAN.  Here is the code:
```{r ex1,echo=FALSE,eval=FALSE}
ccaMatrixList = list( 
  scale( X, center=F, scale=F ), 
  scale( Y, center=F, scale=F ) )
cth = 50
sccanToy = sparseDecom2( ccaMatrixList, inmask = c( maskX, maskY ), its = 10,
  sparseness = c( 0.05, 0.1 ), nvecs = 4, smooth = 1.0, cthresh = c( cth, cth ), 
  perms = 50 )
sccanToy$ccasummary
```
The parameters passed to the software relate directly to the optimization criterion.  More specifically:

* The `sparseness` entry relates to the terms $\gamma_x, \gamma_y$. However, for user convenience, the units are expressed in terms of target percent sparseness (relative to the size of the mask).
* The `smooth` and `cthresh` entries relate to the *operator* within the $\ell_1$ penalty term.  Above, we set $G_1.0^5$ where the numerator sets isolated voxel clusters below that size to zero.  Larger settings of $t$ lead to pseudo-eigenvectors that tend toward greater spatial connectivity.
* `nvecs` sets the number of pseudo-eigenvectors to compute.
We also set `perms = 20` which forces the algorithm to internally permute the data and recompute 20 times, comparing the original to the permuted correlations.  This provides an empirical, if conservative, estimate of significance for each pseudo-eigenvector pair.

```{r rirl,echo=FALSE,eval=FALSE}
myevX1 = makeImage( maskX, abs( sccanToy$eig1[,2] ) ) %>%
  resampleImageToTarget( ref ) %>% iMath("Normalize")
myevY1 = makeImage( maskY, abs( sccanToy$eig2[,2] ) ) %>%
  resampleImageToTarget( ref ) %>% iMath("Normalize")
plot( ref, list( myevX1, myevY1 ), 
...
Finally, let us compare to "classic" cca wherein -- using the same methods -- we see that a dense eigenvector results.
